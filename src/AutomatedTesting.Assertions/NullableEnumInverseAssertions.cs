...
/// <summary>
/// Inverse assertions for nullable <see cref="Enum"/> values.
/// </summary>
/// <typeparam name="T"> The nullable enumeration's type. </typeparam>
public sealed class NullableEnumInverseAssertions<T> : IFluentInterface
    where T : struct, Enum
{
    #region Dependencies

    /// <summary>
    /// Creates a new instance of the <see cref="NullableEnumInverseAssertions{T}"/> type.
    /// </summary>
    /// <param name="nullableEnumeration"> The nullable enumeration to be validated. </param>
    public NullableEnumInverseAssertions(Nullable<T> nullableEnumeration)
    {
    ...
    /// <summary>
    /// Assert that a given nullable enumeration value does not equal an <paramref name="expected"/> value.
    /// </summary>
...
#nullable restore
    ...
    /// <summary>
    /// Assert that a given nullable enumeration value does not have an <paramref name="expected"/> flag.
    /// </summary>
...
#nullable restore
    ...
    /// <summary>
    /// Assert that a given nullable enumeration value is not null.
    /// </summary>
    /// <param name="because"> A reason why this assertion needs to be correct. </param>
    /// <param name="testMethodName"> Supplied by the compiler. </param>
    /// <param name="lineNumber"> Supplied by the compiler. </param>
...
#nullable restore
    {
        if (NullableEnumeration == null)
        {
            ...
            var actual = FormattableString.Invariant($"is \"{NullableEnumeration}\"");
            var expectedText = FormattableString.Invariant($"not to be null");
            throw Context.GetFormattedException(testMethodName, context, actual, expectedText, because);
        }
    }
